...
class PatientDataset(Dataset):
	...
	self.patch_positions.append(self.h5_ids)
	self.num_patches += len(self.h5_ids)
	...
	if index//8 < len(self.slide_ids):
	    ...
	    with h5py.File(self.h5_path, 'r') as h5:
                ...
                img = np.array(h5[self.patch_positions[index//8]].get('rawimage'))
                ...
                for labelname in self.labels.keys():
                    if labelname in h5[self.patch_positions[index//8]].keys():
                        mask = np.array(h5[self.patch_positions[index//8]].get(labelname))
                        labelmap[mask > 0] = self.labels[labelname]

		patch = torch.from_numpy(patch / 255).permute((2, 0, 1)).float().cuda()
		conds = torch.from_numpy(masks).float().unsqueeze(0).cuda()
